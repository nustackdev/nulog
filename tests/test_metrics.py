"""Metric writes + reads: observe, range, sample."""
...
def test_observe_returns_nu_without_executing(ctx):
    ...
    assert _read(ctx, nulog.range_metric("cpu", 0, 10**17)) == []
...
def test_observe_with_explicit_ts_and_range(ctx):
    ...
    rows = _read(ctx, nulog.range_metric("cpu", begin, end))
...
def test_observe_defaults_to_eval_time(ctx):
    ...
    rows = _read(ctx, nulog.range_metric("gauge", 0, 10**17))
    assert len(rows) == 1
...
def test_series_are_isolated(ctx):
    ...
    a_rows = _read(ctx, nulog.range_metric("a", 0, 10**12))
    b_rows = _read(ctx, nulog.range_metric("b", 0, 10**12))
...
def test_sample_returns_at_most_n(ctx):
    ...
    rows = _read(ctx, nulog.sample_metric("burst", 5, begin, end))
    assert len(rows) <= 5
...
def test_range_out_of_bounds_is_empty(ctx):
    ...
    assert _read(ctx, nulog.range_metric("m", 0, 1000)) == []
    assert _read(ctx, nulog.sample_metric("m", 3, 0, 1000)) == []
...
def test_compose_metric_and_log_is_atomic(ctx):
    ...
    m = _read(ctx, nulog.range_metric("throughput", 0, 10**17))
...
def test_loop_mints_fresh_metric_key(ctx):
    ...
    rows = _read(ctx, nulog.range_metric("hb", 0, 10**17))
    assert len(rows) == 4
